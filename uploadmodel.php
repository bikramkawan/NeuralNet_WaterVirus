...
<?php
  ...
  $target_path = "Matlab/models/";

  for($i=1;$i<=5;$i++) {
    $attachments = 'attach_'.$i;
    $attachmentdiv = 'attachdiv_'.$i;
    $FileName = $_FILES[$attachments]['name'];
        // echo "<script type='text/javascript'>var filename='".$attachmentdiv."';
    ...
    if($FileName != "") {
      ...
      if($FileExtension != "m" && $FileExtension != "xlsx" && $FileExtension != "png" && $FileExtension != "txt") {
        ...
        if($FileSize > 4000) {
          ...
          if(move_uploaded_file($FileTemp,$FileLocation)) {
            ...
            echo "<script type='text/javascript'>var filename='".$FileName."';
            alert(filename);
            </script>";
            echo "<div id="'bbbikram'" >ffffffffff</div>";
